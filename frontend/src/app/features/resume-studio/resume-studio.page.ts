            ...
            <section class="grid gap-5 md:grid-cols-2">
              <div class="rounded-2xl border border-brand-200/70 bg-white/80 p-6 shadow-sm dark:border-brand-700/60 dark:bg-brand-900/40">
                ...
                @if (githubReposLoading()) {
                  <p class="mt-3 text-sm opacity-70">Loading projects from GitHub...</p>
                  ...
                  <div class="mt-3 grid gap-3">
                    @for (repo of githubRepos(); track repo.id) {
                      <a
                        ...
                        class="block rounded-xl border border-brand-200/80 p-4 no-underline dark:border-brand-700"
                      >
                        <p class="m-0 text-sm font-semibold">{{ repo.name }}</p>
                        <p class="m-0 mt-1 text-sm leading-6 opacity-90">{{ repo.description || 'No description provided.' }}</p>
                        <div class="mt-2 flex flex-wrap items-center gap-2">
                          @if (repo.language) {
                            <span class="rounded-full bg-brand-100 px-2 py-1 text-xs dark:bg-brand-800/60">{{ repo.language }}</span>
                          }
                          @if (repo.stargazers_count > 0) {
                            <span class="text-xs opacity-70">★ {{ repo.stargazers_count }}</span>
                          }
                        </div>
                      </a>
                    }
